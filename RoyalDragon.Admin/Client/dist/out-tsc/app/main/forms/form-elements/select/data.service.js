...
let DataService = class DataService {
    constructor(http) {
        this.http = http;
        this._gitHubAccountsCache = new Map();
    }
    getGithubAccounts(term = null) {
        ...
        if (term) {
            return this.http.get(`https://api.github.com/search/users?q=${term}`).pipe(map(rsp => rsp.items), tap(data => this._gitHubAccountsCache.set(term, data)));
        }
        else {
            return of([]);
        }
