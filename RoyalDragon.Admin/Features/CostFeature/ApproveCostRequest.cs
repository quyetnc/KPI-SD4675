...
{
    public class ApproveCostRequest : IRequest<ResultCustomModel<bool>>
    {
        public Cost? Cost { get; set; }
        public int? UserId { get; set; }
        public class QueryHandler : IRequestHandler<ApproveCostRequest, ResultCustomModel<bool>>
        {
            ...
            public class CommandValidator : AbstractValidator<ApproveCostRequest>
            {
            ...
            private async Task<string> GetFullNameByUserId(int? userId) =>await _db.User.Where(x=>x.UserId==userId).Select(x=>x.Fullname).FirstOrDefaultAsync();
            public async Task<ResultCustomModel<bool>> Handle(ApproveCostRequest request, CancellationToken cancellationToken)
            {
                ...
                cost.IsApprove = true;
                cost.ApproveBy =await GetFullNameByUserId(request.UserId);
