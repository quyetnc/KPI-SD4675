...
export class CommonService {
  constructor(@Inject(DOCUMENT) private _config: CoreConfigService) {
  ...
  // <!-- star = 1.5 , i 3-->
  fRenderClassRating(i: number, star: number): string {
    let ratingHtml: string = '';
    let isStarHalf: boolean = false;
    if (i <= star)
      ratingHtml = 'fas fa-star checked';
    else {
      //Nếu là số chẳn
      if (Number.isInteger(star)) {
        ratingHtml = 'fas fa-star';
      } else {
        if (isStarHalf == false) {
          isStarHalf = true;
          ratingHtml = 'fas fa-star-half-alt checked';
        }
        else
          ratingHtml = 'fas fa-star';
      }
