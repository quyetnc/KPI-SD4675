...
namespace RoyalDragon.Common.Core
{
    ...
    public static class Extension
    {
        ...
        public static int GetBlockNumber(this string input)
        {
            if (input == null)
                return 0;
            var test = Regex.Match(input, "<a href='\\/block\\/.*?'>.*?<\\/a>");
            string[] arr = test.Value.Split('>');
            string val = "";
            if (arr.Length > 1)
                val = arr[1];
            string blockNumber = "";
        ...
        public async static Task<int> GetTokenTransfers(this string input)
        {
            if (input == null)
                return 0;
            string html = await API.GetStringAsync($"https://testnet.bscscan.com/tx/{input}");
            if (html == null)
                return 0;
            string regex = Regex.Match(html, "<b>To.*?<img").Value.Replace(" </span><img", "");
            string resultString = "";
            for (int i = regex.Length - 1; i >= 0; i--)
            {
                ...
                {
                    if (char.IsDigit(regex[i]))
                        resultString += regex[i];
                }
            }
            if (resultString != "")
            {
        ...
        /// <summary>
        /// Hàm chuyển từ Hex qua Number
        /// </summary>
        /// <param name="hex"></param>
        /// <returns></returns>
        public static int HexToInt(this HexBigInteger hex)
        {
            if (hex is null)
            {
                return 0;
            }
            return Convert.ToInt32(hex.ToString().Replace("0x", ""), 16);
        }
        ...
        /// <param name="hex"></param>
        /// <returns></returns>
        public static decimal HexToDecimal(this HexBigInteger hex)
        {
            return Web3.Convert.FromWei(hex);
        }
        /// <summary>
        /// Hàm chuyển từ Hex qua Number
        /// </summary>
        /// <param name="hex"></param>
        /// <returns></returns>
        public static BigInteger HexToDecimal(this string hex)
        {
            return Web3.Convert.ToWei(hex);
        }
        ...
        public static async Task<GridReader> QueryMultipleAsync(this SqlConnection con, string sql, object param = null)
        {
